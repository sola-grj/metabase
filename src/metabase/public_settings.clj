...
(defsetting site-url
  ...
  (deferred-tru "此平台实例的基本URL，例如“http://localhost:9000”。")
  :visibility :public
  :getter (fn []
            (try
...
(defsetting anon-tracking-enabled
  ...
  (deferred-tru "启用匿名使用数据的收集，以帮助改进平台。")
  :type       :boolean
  :default    true
  :visibility :public)
...
(defsetting query-caching-min-ttl
  ...
  (deferred-tru "平台会捕捉所有平均查询时间大于这么多秒的已保存问题")
  :type    :double
  :default 60)
...
(defsetting query-caching-ttl-ratio
  ...
  (str (deferred-tru "为了确定每个保存的问题的缓存结果应该保留多长时间，我们取查询的平均执行时间，然后乘以此处输入的值。")
       " "
       ...
       (deferred-tru "因此，如果一个查询平均运行2分钟，并且您为乘数输入10，那么它的缓存条目将持续20分钟。"))
  :type    :integer
  :default 10)
